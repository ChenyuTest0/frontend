  ...
  >
    <div class="relative mx-auto my-6 w-auto max-w-sm">
      ...
      >
        ...
        >
          <button
            class="background-transparent mr-1 mb-1 px-6 py-2 text-sm font-bold text-red-500 uppercase outline-hidden transition-all duration-150 ease-linear focus:outline-hidden"
            type="button"
            @click="popModal"
          >
            {{ $t('Button.OK') }}
...
const popModal = () => {
  deactivate();
  if (props.applicationError?.errorHandlerAfterCallback) {
    props.applicationError.errorHandlerAfterCallback().finally(() => {
      ModalUtil.pop();
    });
    return;
  }
